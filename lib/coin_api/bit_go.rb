...
module CoinAPI
  class BitGo < BaseAPI
    ...
    def create_address!(options = {})
      if options[:address_id].present?
        ...
        Rails.logger.info "Create Address response from BitGo: #{response.inspect}"
    ...
    def create_withdrawal!(issuer, recipient, amount, options = {})
      ...
      }).fetch('txid').tap(&method(:normalize_txid))
    end
  ...
  private
    ...
    def build_deposit_entries(tx)
      entries = tx.fetch('entries')
        .map do |entry|
          next unless entry['wallet'] == wallet_id
          next unless entry['valueString'].to_d > 0
          next if currency.code.eql? "xrp" && tx['destinationTag'].blank?
          next if entry.key?('outputs') && entry['outputs'] != 1
          entry
        end
        .compact
        entries.each do |entry|
          entry[:amount] = convert_from_base_unit(entry.fetch('valueString'))
          entry[:address] = normalize_address(entry.fetch('address')) 
        end
	Rails.logger.info "Entries: #{entries.inspect}"
	return entries
    end
